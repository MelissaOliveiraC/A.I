...
function buscarCaminho(origem, destino, possibilidades) {
    ...
    while (fila.length) {
      ...
      possibilidades[no]
        //para cada vizinho não visitado, includes testa se está no array, negando a lógica "!" -> filtra-se apenas vizinhos não estão em visitados.  
        .filter(vizinho => !visitados.includes(vizinho))

        //adiciona na lista de visitados, na fila define nó-atual como parente desse vizinho.
        .forEach(vizinho => {
