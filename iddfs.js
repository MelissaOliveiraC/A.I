const iddfs = (grafo, inicio, meta, limiteProfundidade) => {
    for (let limite = 0; limite <= limiteProfundidade; limite++) {
        if (dls(grafo, inicio, meta, limite)) {
...
const encontrouDestino = iddfs(grafo, origem, destino, limiteProfundidade);
